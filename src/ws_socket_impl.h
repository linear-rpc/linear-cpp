...
class WSSocketImpl : public linear::SocketImpl {
 public:
  ...
  void OnConnect(const linear::shared_ptr<linear::SocketImpl>& socket, tv_stream_t* handle, int status);
  bool CheckRetryAuth();
  ...
  void SetWSResponseContext(const linear::WSResponseContext& response_context);
