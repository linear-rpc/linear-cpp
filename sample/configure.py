...
sys.path.insert(0, os.path.join(linear_root, 'gyp', 'pylib'))
try:
  import gyp
except ImportError:
  print('not found: gyp')
...
if __name__ == '__main__':
  ...
  if sys.platform == 'win32':
    args.append(os.path.join(linear_root, 'linear-sample.gyp'))
    ...
    args.append(os.path.join(os.path.abspath(linear_root), 'linear-sample.gyp'))
  ...
  if not any(a.startswith('-Dwith_linear=') for a in args):
    args.append('-Dwith_linear=..\\')
